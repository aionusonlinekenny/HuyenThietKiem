...
typedef struct PLAYER_TEAM_ADD_MEMBER_DATA
{
	...
	int 	m_MHP;
	int 	m_TP;
	int 	m_SubWorldID;     // Th�m bien SubWorldID
...
typedef struct
{
    ...
    BYTE SrcY;
    BYTE DestPlace;
    BYTE DestX; // client ?ang ?? -1 ?? server t? t�m
    BYTE DestY;
...
/*typedef struct
{
    ...
    int  nValue;        // Gi� tr? hi?n th? Li�n Tr?m (v� d?: id tr?m, ho?c 1/0)
} S2C_SHOW_LIEN_TRAM;*/
typedef struct
{
	BYTE	ProtocolType;		
...
// CHANGED: Increased from 1024 to 2048 bytes to support longer dialog text
// WARNING: Server and Client MUST use same value! Rebuild both when changing!
#define MAX_SCIRPTACTION_BUFFERNUM 2048
