...
typedef struct PLAYER_TEAM_ADD_MEMBER_DATA
{
	...
	int 	m_MHP;
	int 	m_TP;
	int 	m_SubWorldID;     // ThÍm bien SubWorldID
...
typedef struct
{
    BYTE ProtocolType; // c2s_rightitemautomove
    BYTE SrcPlace;
    BYTE SrcX;
    BYTE SrcY;
    BYTE DestPlace;
    BYTE DestX; // client ?ang ?? -1 ?? server t? tÏm
    BYTE DestY;
} ITEM_RIGHT_AUTO_MOVE_REQ;

typedef struct
{
    BYTE ProtocolType;   // s2c_rightitemautomove
    BYTE SrcPlace;
    BYTE SrcX;
    BYTE SrcY;
    BYTE DestPlace;
    ...
    BYTE DestY;
    int  ItemID; 
} ITEM_RIGHT_AUTO_MOVE;
/*typedef struct
{
    BYTE ProtocolType;  // = s2c_show_lien_tram
    int  nValue;        // Gi· tr? hi?n th? LiÍn Tr?m (vÌ d?: id tr?m, ho?c 1/0)
} S2C_SHOW_LIEN_TRAM;*/
typedef struct
{
	BYTE	ProtocolType;		
...
typedef struct
{
	BYTE			ProtocolType;		// –≠“È¿‡–Õ
	DWORD			dwID;
	DWORD			dwTimePacker;
	int				m_nTargetPlayerIdx;	// ±ªΩ” ‹player idx
} CHAT_ADD_FRIEND_COMMAND;				// ÃÌº”ƒ≥ÕÊº“Œ™¡ƒÃÏ∫√”—
...
typedef struct
{
	...
	DWORD			dwID;
	DWORD			dwTimePacker;
	int				m_nTargetPlayerIdx;	// ±ªæ‹æ¯player idx
} CHAT_REFUSE_FRIEND_COMMAND;			// æ‹æ¯ÃÌº”ƒ≥ÕÊº“Œ™¡ƒÃÏ∫√”—

typedef struct
{
	BYTE			ProtocolType;		// –≠“È¿‡–Õ
	DWORD			m_dwID;				// –¬ÃÌº”∫√”—µƒ id
	int				m_nIdx;				// –¬ÃÌº”∫√”—‘⁄ player  ˝◊È÷–µƒŒª÷√
	char			m_szName[32];		// –¬ÃÌº”∫√”—µƒ√˚◊÷
} CHAT_ADD_FRIEND_SYNC;					// Õ®÷™øÕªß∂À≥…π¶ÃÌº”“ª∏ˆ¡ƒÃÏ∫√”—

typedef struct
{
	BYTE			ProtocolType;		// –≠“È¿‡–Õ
	WORD			m_wLength;
	char			m_szName[32];		// æ‹æ¯’ﬂ√˚◊÷
} CHAT_REFUSE_FRIEND_SYNC;				// Õ®÷™øÕªß∂ÀÃÌº”¡ƒÃÏ∫√”—µƒ…Í«Î±ªæ‹æ¯

typedef struct
{
	BYTE			ProtocolType;		// –≠“È¿‡–Õ
	int				m_nTargetPlayerIdx;	// ≥ˆ¥Ì player idx (“ª∞„ø…ƒ‹ «¥Àplayerœ¬œﬂªÚ’ﬂªª∑˛ŒÒ∆˜¡À)
} CHAT_ADD_FRIEND_FAIL_SYNC;			// Õ®÷™øÕªß∂ÀÃÌº”¡ƒÃÏ∫√”— ß∞‹
...
} CHAT_LOGIN_FRIEND_NONAME_SYNC;		// ÕÊº“µ«¬º ±∑¢ÀÕÕÊº“¡ƒÃÏ∫√”— ˝æ›£®≤ª¥¯√˚◊÷£©
...
typedef struct
{
	...
	int				m_nPlayerIdx;		// ∫√”— player index
	char			m_szName[32];		// ∫√”—√˚◊÷
} CHAT_LOGIN_FRIEND_NAME_SYNC;			// ÕÊº“µ«¬º ±∑¢ÀÕÕÊº“¡ƒÃÏ∫√”— ˝æ›£®¥¯√˚◊÷£©
...
typedef struct
{
	...
	DWORD			dwTimePacker;
	DWORD			m_dwID;				// ∫√”— id	
} CHAT_APPLY_SEND_ONE_FRIEND_NAME_COMMAND;	// …Í«Îµ√µΩÕÊº“ƒ≥∏ˆ¡ƒÃÏ∫√”—µƒÕÍ’˚ ˝æ›
...
typedef struct
{
	...
	int				m_nPlayerIdx;		// ∫√”— player index
	char			m_szName[32];		// ∫√”—√˚◊÷
} CHAT_ONE_FRIEND_DATA_SYNC;			// ∑¢ÀÕÕÊº“ƒ≥“ª∏ˆ¡ƒÃÏ∫√”— ˝æ›£®¥¯√˚◊÷£©
...
typedef struct
{
	...
	DWORD			m_dwID;				// ∫√”— id
	int				m_nPlayerIdx;		// ∫√”— player index
} CHAT_FRIEND_ONLINE_SYNC;				// Õ®÷™øÕªß∂À”–∫√”—…œœﬂ
...
typedef struct
{
	...
	DWORD			dwTimePacker;
	DWORD			m_dwID;				// ±ª…æ≥˝id
} CHAT_DELETE_FRIEND_COMMAND;			// …æ≥˝ƒ≥∏ˆ¡ƒÃÏ∫√”—

typedef struct
{
	BYTE			ProtocolType;		// –≠“È¿‡–Õ
	DWORD			m_dwID;				// …æ≥˝id
} CHAT_DELETE_FRIEND_SYNC;				// ±ªƒ≥∏ˆ¡ƒÃÏ∫√”—…æ≥˝
...
typedef struct
{
	...
	DWORD			dwTimePacker;
	DWORD			m_dwID;				// ±ª…æ≥˝id
} CHAT_REDELETE_FRIEND_COMMAND;			// …æ≥˝ƒ≥∏ˆ¡ƒÃÏ∫√”—
