...
void KItem::GetDesc(char* pszMsg, bool bShowPrice, int nPriceScale, int nActiveAttrib)
{
	...
	{	
		char szTmp[16];
		sprintf(szTmp, " (C�p %d)", m_CommonAttrib.bLevel);
		strcat(pszMsg, szTmp);
	}
		...
		m_CommonAttrib.cGenre == item_script)
	{
		...
		{
			strcat(pszMsg, "\n");
			strcat(pszMsg, "<color=red>V�t ph�m kh�ng th� b�n, giao d�ch <color>");
		}
	}
	//gioi han
	if(m_CommonAttrib.cGenre == item_script)
	{
		if(m_CommonAttrib.nTaskIdScript > 0)
		{
			...
			{
				sprintf(szTmp, "<color=Green>�� d�ng h�t %d v�t ph�m h�m nay<color>",m_CommonAttrib.nTaskIdScriptMax);
			}else{
				sprintf(szTmp, "<color=Green>Gi�i h�n trong ng�y: %d/%d <color>", j,m_CommonAttrib.nTaskIdScriptMax);
			}
			strcat(pszMsg, szTmp);
			strcat(pszMsg, "\n");
		}
	...
	{
		strcat(pszMsg, "\n");
		strcat(pszMsg, "<color=green>V�t ph�m �� kh�a b�o hi�m<color>");
	}
	else if(m_dwBindState == 2)
	{
		strcat(pszMsg, "\n");
		strcat(pszMsg, "<color=green>V�t ph�m ��nh k�m nh�n v�t<color>");
	}
	...
	{	
		...
		switch(m_btPriceType)
		{
			case 1:
				...
				break;
			case 3:
				GetPriceDesc(-1, nPriceScale, "�i�m", "<color=violet>", szPrice);
				break;
			default:
				GetPriceDesc(-1, nPriceScale, "l��ng", "<color=yellow>", szPrice);
				break;
		}
	...
	{	
		...
		{
			case series_metal:
				strcat(pszMsg, "\n<color=White>Thu�c t�nh ng� h�nh: <color=Metal>Kim");
				break;
			case series_wood:
				strcat(pszMsg, "\n<color=White>Thu�c t�nh ng� h�nh: <color=Wood>M�c");
				break;
			case series_water:
				strcat(pszMsg, "\n<color=White>Thu�c t�nh ng� h�nh: <color=Water>Th�y");
				break;
			case series_fire:
				strcat(pszMsg, "\n<color=White>Thu�c t�nh ng� h�nh: <color=Fire>H�a");
				break;
			case series_earth:
				strcat(pszMsg, "\n<color=White>Thu�c t�nh ng� h�nh: <color=Earth>Th� ");
				break;
		}
	}
	...
	if(m_CommonAttrib.cGenre == item_mine)
	{
		if(m_CommonAttrib.bLevel > 0)
		{
			char szTmp[16];
			strcat(pszMsg, "\n");
			sprintf(szTmp, "<color=purple>Ph�m ch�t: %d <color>", m_CommonAttrib.bLevel);
			strcat(pszMsg, szTmp);
			strcat(pszMsg, "\n");
		}
	}
	  ...
	  )
	{
		for(i = 0; i < 7; i++)
		{
			...
			if (m_aryBaseAttrib[i].nAttribType == magic_durability_v)
			{
				...
				{
					char	szDurInfo[32];
					sprintf(szDurInfo, "<color=red>Trang b� t�n h�i<color>");
				...
				else
				{
					char	szDurInfo[32];
					if (m_nCurrentDur == -1)
						sprintf(szDurInfo, "<color=Yellow>Kh�ng th� ph� h�y<color=White>");
					else
						sprintf(szDurInfo, "�� b�n: %3d / %3d", GetDurability(), GetMaxDurability());
					strcat(pszMsg, szDurInfo);
				}
			}
			else
	...
	if(m_CommonAttrib.cGenre == item_script)
	{
		...
		{	
			char szInfo[64];
			sprintf(szInfo, "C� th� s� d�ng:<color=green> %d  <color>l�n.", this->GetDurability());
			strcat(pszMsg, "\n");
		...
		switch(m_CommonAttrib.nDetailType)
		{
			...
			case equip_helm:
				...
				if(Item[nIndex].GetID() == this->m_dwID && this->m_dwID != 0)
				{
					...
					strcat(pszMsg, "c�a <color=green>D�y Chuy�n<color> v� <color=green>Y Ph�c<color> �� k�ch thu�c t�nh �n<color>\n");
					strcat(pszMsg, "\n");
				}
				break;
			case equip_armor:
			case equip_amulet:	
				...
				{
					strcat(pszMsg, GetRequireSeries(m_CommonAttrib.cSeries));
					strcat(pszMsg, "c�a <color=green>Nh�n D��i<color> v� <color=green>Th�t L�ng<color> �� k�ch thu�c t�nh �n<color>\n");
					strcat(pszMsg, "\n");
				}
				break;
			case equip_ring:
			...
			case equip_belt:
				{	
					...
					{	
						strcat(pszMsg, GetRequireSeries(m_CommonAttrib.cSeries));
						strcat(pszMsg, "c�a <color=green>M� <color>v� <color=green>V� Kh� <color>�� k�ch thu�c t�nh �n<color>\n");
						strcat(pszMsg, "\n");
					...
					{	
						strcat(pszMsg, GetRequireSeries(m_CommonAttrib.cSeries));
						strcat(pszMsg, "c�a <color=green>Ng�c B�i<color> v� <color=green>Bao Tay<color> �� k�ch ho�t thu�c t�nh �n\n");
						strcat(pszMsg, "\n");
			...
			case equip_pendant:	
				...
				{	
					strcat(pszMsg, GetRequireSeries(m_CommonAttrib.cSeries));
					strcat(pszMsg, "c�a <color=green>Gi�y<color> v� <color=green>Nh�n Tr�n<color> �� k�ch thu�c t�nh �n<color>\n");
					strcat(pszMsg, "\n");
				}
				break;
			default:
	...
	{	
		...
		switch(m_btPriceType)
		{
			case 1:
				...
				break;
			default:
				GetPriceDesc(0, nPriceScale, "l��ng", "<color=yellow>", szPrice);
				break;
		}
		strcat(pszMsg, szPrice);
	}
	...
	{
		char szBuffer[32];
		sprintf(szBuffer, "<color=fire>Th�i gian b�n: %02d ng�y 00 gi� 00 ph�t", 
				m_btDaySell);
		strcat(pszMsg, "\n\n");
		strcat(pszMsg, szBuffer);
		strcat(pszMsg, "\n");
	...
	{	
		...
		{	
			sprintf(szInfo, "<color=yellow>Gi� ni�m y�t: %d l��ng<color>", m_nShopPrice);
		}
		else
		{
			...
			if(!nMod)
			{	
				sprintf(szInfo, "<color=yellow>Gi� ni�m y�t: %d v�n l��ng<color>", nDivisor);
			}
			else
			{	
				sprintf(szInfo, "<color=yellow>Gi� ni�m y�t: %d v�n %d l��ng<color>", nDivisor, nMod);
			}
		}
		strcat(pszMsg, "\n");
		strcat(pszMsg, szInfo);
...
char* KItem::GetRequireSeries(char cSeries) const
{
	...
	{
		case series_metal:
			return "<color=Yellow>C�n h� <color=earth>(Th�)<color> ";
			break;
		case series_wood:
			return "<color=Yellow>C�n h� <color=water>(Th�y)<color> ";
			break;
		case series_water:
			return "<color=Yellow>C�n h� <color=metal>(Kim)<color> ";
			break;
		case series_fire:
			return "<color=Yellow>C�n h� <color=wood>(M�c)<color> ";
			break;
		case series_earth:
			return "<color=Yellow>C�n h� <color=fire>(H�a)<color> ";
			break;
		default:
			return "";
			break;
...
{	
	...
	else if(cDisCount == 0)
	{
		...
		{	
			sprintf(lpszPrice, "<color=red>Gi� c�: %d %s<color>\n", m_CommonAttrib.nPrice/* / nPriceScale*/, pszUnit);
		}
		else
		{
			...
			if(!nMod)
			{	
				sprintf(lpszPrice, "<color=red>Gi� c�: %d v�n %s<color>", nDivisor, pszUnit);
			}
			else
			{	
				sprintf(lpszPrice, "<color=red>Gi� c�: %d v�n %d %s<color>", nDivisor, nMod, pszUnit);
			}
		}
	}
}
...
{	
	...
	{	
		if(bBind == TRUE)
		{
			sprintf(pszTime, "<color=green>Th�i gian m� kh�a: %02d:%02d %02d-%02d-%02d<color>\n", nHour, nMin, nDate, nMonth, nYear + 2000);
		}
		else
		{
			sprintf(pszTime, "<color=fire>Th�i h�n s� d�ng:<color> <color=green>%02d:%02d %02d-%02d-%02d \n", nHour, nMin, nDate, nMonth, nYear + 2000);
