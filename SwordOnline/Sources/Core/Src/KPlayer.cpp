	...
	case 4: 
		{
			...
			// Allow ExeTremble, ExeUpgradeAttrib, ExeCompoundForge, ExeCompoundEquipment, ExeCompoundCrystal, and ExeExtractAttribute functions
			...
			{
				g_DebugLog("[SERVER KPLAYER] ERROR: Function '%s' not allowed! Only ExeTremble, ExeUpgradeAttrib, ExeCompoundForge, ExeCompoundEquipment, ExeCompoundCrystal, ExeExtractAttribute allowed", pExe->m_szContent);
				break;
			}
			...
			else if( strcmp(pExe->m_szContent, "ExeCompoundCrystal") == 0 )
			{
				g_GameSettingFile.GetString("COMPOUNDFORGE", "Script", "", szScriptFile, sizeof(szScriptFile));
				g_DebugLog("[SERVER KPLAYER] ExeCompoundCrystal: Script file = %s", szScriptFile);
			}
			else if( strcmp(pExe->m_szContent, "ExeExtractAttribute") == 0 )
			{
				g_GameSettingFile.GetString("COMPOUNDFORGE", "Script", "", szScriptFile, sizeof(szScriptFile));
				g_DebugLog("[SERVER KPLAYER] ExeExtractAttribute: Script file = %s", szScriptFile);
			}
			else
			{
