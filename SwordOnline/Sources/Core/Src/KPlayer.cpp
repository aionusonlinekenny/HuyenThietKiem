...
void	KPlayer::AddSkillPoint(BYTE* pProtocol)
{
	...
	if (m_nSkillPoint >= pAdd->m_nAddPoint && nSkillIndex )
	{
		...
		{	
			...
			// Re-read PointAdd after level increase to get accurate value.
			// Needed when a new skill (PointAdd=0) receives its first point,
			// so the client gets the correct bonus from currently equipped items.
			nPointAdd = Npc[m_nIndex].m_SkillList.GetSkillIdxPointAdd(nSkillIndex);
	...
	case 4:
		{
			...
			else if( strcmp(pExe->m_szContent, "ExeEnchaseAttribute") == 0 )
			{
				g_GameSettingFile.GetString("COMPOUNDFORGE", "Script", "", szScriptFile, sizeof(szScriptFile));
				g_DebugLog("[SERVER KPLAYER] ExeEnchaseAttribute: Script file = %s", szScriptFile);
			}
			else
			{
