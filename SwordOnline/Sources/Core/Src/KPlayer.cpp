...
void	KPlayer::Release()
{
	...
	m_dwLastPosSyncTime = 0;  // Initialize periodic sync timer
	m_MouseDown[0] = FALSE;
	m_MouseDown[1] = FALSE;
	m_bDebugMode	= FALSE;
...
void KPlayer::ProcessMouse(int x, int y, int Key, MOUSE_BUTTON nButton)
{
	...
	if (Key == 0 )
	{
		...
		if (m_nSendMoveFrames >= defMAX_PLAYER_SEND_MOVE_FRAME)
		{
			...
			m_nSendMoveFrames = 0;
			m_dwLastPosSyncTime = GetTickCount();  // Update last sync time when sending move command
		}
		return;
	}
}
...
void KPlayer::Walk(int nDir, int nSpeed)
{
	...
	if (m_RunStatus)
	{
		...
		SendClientCmdRun(nX, nY);
		m_dwLastPosSyncTime = GetTickCount();  // Update last sync time
	}
	else
	{
		Npc[m_nIndex].SendCommand(do_walk, nX, nY);
