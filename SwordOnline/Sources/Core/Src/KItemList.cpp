...
// Auto-sort equipment room - compact items from left to right, top to bottom
...
#ifndef _SERVER
// Auto-sort equipment room - compact items from left to right, top to bottom
// SAFE VERSION: Uses inventory swap instead of pickup/place
void KItemList::AutoSortEquipment()
{
	// Use static flag to prevent recursive calls
	static BOOL s_bInAutoSort = FALSE;
	if (s_bInAutoSort)
		return;
	...
	// Simple bubble-sort approach: swap items to compact them
	// This doesn't require pickup/place, just swaps grid positions
	int nX, nY;
	BOOL bChanged = FALSE;

	for (nY = 0; nY < EQUIPMENT_ROOM_HEIGHT; nY++)
	{
		for (nX = 0; nX < EQUIPMENT_ROOM_WIDTH; nX++)
		{
			int nIdx = m_Room[room_equipment].FindItem(nX, nY);
			if (nIdx > 0)
			{
				// Find the leftmost topmost position this item can move to
				POINT bestPos;
				bestPos.x = nX;
				bestPos.y = nY;
				...
				// Scan from top-left to find better position
				int testY, testX;
				for (testY = 0; testY < EQUIPMENT_ROOM_HEIGHT; testY++)
				{
					for (testX = 0; testX < EQUIPMENT_ROOM_WIDTH; testX++)
					{
						// Skip if this is current position
						if (testX == nX && testY == nY)
							continue;

						// Skip if this position comes after current best
						if (testY > bestPos.y || (testY == bestPos.y && testX >= bestPos.x))
							continue;

						// Check if item fits here
						if (m_Room[room_equipment].CheckRoom(testX, testY, nWidth, nHeight))
						{
							bestPos.x = testX;
							bestPos.y = testY;
							break;
					...
					if (bestPos.x != nX || bestPos.y != nY)
						break;
				}

				// Move item if better position found
				if (bestPos.x != nX || bestPos.y != nY)
				{
					// Remove from old position
					m_Room[room_equipment].PickUpItem(nIdx, nX, nY, nWidth, nHeight);
					// Place at new position
					m_Room[room_equipment].PlaceItem(bestPos.x, bestPos.y, nIdx, nWidth, nHeight);

					// Update item position in list
					int nListIdx = FindSame(nIdx);
					if (nListIdx > 0)
					{
						m_Items[nListIdx].nX = bestPos.x;
						m_Items[nListIdx].nY = bestPos.y;
					}
