...
{	
	...
	// {
		...
		// {	
			...
			// sprintf(szMsg, "�� m� kh�a b�o hi�m v�t ph�m <color=green>[%s]<color>.", Item[nIndex].GetName());
			// KPlayerChat::SendSystemInfo(1, m_PlayerIdx, "V�t ph�m", szMsg, strlen(szMsg));
		// }
		// else
		// {	
			// char szMsg[512];//80  //luu y do dai cua ky tu khi build release by kinnox;
			// sprintf(szMsg, "�� kh�a b�o hi�m v�t ph�m <color=green>[%s]<color>.", Item[nIndex].GetName());
			// KPlayerChat::SendSystemInfo(1, m_PlayerIdx, "V�t ph�m", szMsg, strlen(szMsg));
		// }
	// }
	ITEM_CHANGE_INFO sChange;
...
int KItemList::MapPlaceToUIContainer(int nPlace)
{
    ...
    // N?u c?n th� b? sung th�m c�c place kh�c ? ?�y

    default:                   return 0; // 0/NULL: container kh�ng h?p l?
    }
}
#endif // !_SERVER
