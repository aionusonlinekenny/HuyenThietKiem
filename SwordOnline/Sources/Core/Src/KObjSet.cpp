...
int		KObjSet::AddMoneyObj(KMapPos MapPos, int nMoneyNum)
{
	...
	sInfo.m_dwNpcId = 0;//AutoAI by kinnox;
	//end code;
...
int		KObjSet::Add(int nDataID, KMapPos MapPos, KObjItemInfo sItemInfo)
{
	...
	if (sItemInfo.m_szName[0] && strlen(sItemInfo.m_szName) < FILE_NAME_LENGTH)		//
	...
	cObjAdd.m_dwItemID = sItemInfo.m_dwItemID;
...
//// --
// Them Object tu script
//// --
int		KObjSet::Add(int nDataID, int nSubWorld, int nMpsX, int nMpsY, int nDir, int nState, char* szScript)
{		
		int nKind, nNo;

		nKind = GetDataIDKind(nDataID);
		...
		SubWorld[nSubWorld].Mps2Map(nMpsX, nMpsY, &sPos.nRegion, &sPos.nMapX, &sPos.nMapY, &sPos.nOffX, &sPos.nOffY);
		...
		sInfo.m_dwNpcId = 0;//AutoAI by kinnox;
		nNo = Add(nDataID, sPos, sInfo);
		if (nNo == -1)
			return 0;
		if(nDir >= 0)
			Object[nNo].SetDir(nDir);
		if(nState >= 0)
			Object[nNo].SetState(nState);
		Object[nNo].SetScriptFile(szScript);
...
int		KObjSet::ClientAdd(int nID, int nDataID, int nState, int nDir, int nCurFrame, int nXpos, int nYpos, KObjItemInfo sInfo)
{
	...
	{
		Object[nAddIndex].m_nDropState = 1;	
		if (nCurFrame > 0)
	...
	Object[nAddIndex].m_dwItemID = sInfo.m_dwItemID;
	...
	//end
	//g_DebugLog("color = %d",sInfo.m_nColorID);
	if (sInfo.m_szName[0])
...
//	
// --
BOOL KObjSet::ServerAdd(const int nDataID, const int nSubWorld, const int nMpsX, const int nMpsY, const int nDir, const int nState, const char* pScriptFile)
{	
	int				nKind, nNo;
	KMapPos			sPos;
	KObjItemInfo	sInfo;
	...
	if(nNo == -1)
		return FALSE;
	if(nDir >= 0)
		Object[nNo].SetImageDir(nDir);
	if(nState >= 0)
		Object[nNo].SetState(nState);
	Object[nNo].SetScriptFile((char*)pScriptFile);
	return TRUE;
}
#endif
...
int		KObjSet::AddData(int nDataID, int nSubWorld, int nRegion, int nMapX, int nMapY, int nOffX, int nOffY)
{
	...
	nFreeNo = FindFree();
	//
	if (nFreeNo <= 0 || nFreeNo >= MAX_OBJECT)
		return -1;
...
int	KObjSet::SearchObjAt(int nX, int nY, int nRange)
{
	...
	while(1)
	{
		...
		if (Obj_Kind_Box != Object[nIdx].m_nKind
			...
			&& Obj_Kind_Task != Object[nIdx].m_nKind)
			continue;
...
#ifndef _SERVER
int		KObjSet::GetDistanceMpx(int nRx1, int nRy1, int nRx2, int nRy2)
{
	return (int)sqrt((float)((nRx1 - nRx2) * (nRx1 - nRx2) + (nRy1 - nRy2) * (nRy1 - nRy2)));
}
int		KObjSet::AutoGetObjNear(int nX1, int nY1, int nDistance, BOOL bPickItem /* = FALSE */, BOOL bPickItemBlue /*= FALSE*/, 
								BOOL bPickEvent /*= FALSE*/, BOOL bPickMoney /*= FALSE*/, BOOL bPickAll /*= FALSE*/)
{
	int nDistanceMin = nDistance;
	int	nDistancePlayer = 0;
	int nIndexReturn = 0;
	...
	while (1)
	{
		...
		if (Object[nIdx].m_nKind != Obj_Kind_Money &&
			Object[nIdx].m_nKind != Obj_Kind_Item &&
			Object[nIdx].m_nKind != Obj_Kind_Task)
				continue;
		...
		if (!bPickAll)
		{
			...
			if(Object[nIdx].m_nKind == Obj_Kind_Item)
			{
				if(bPickEvent)
				{	
					if(bPickItemBlue)
					{
						if(Object[nIdx].m_nColorID != 3 && Object[nIdx].m_nColorID != 1)
							continue;
					}
					else
					{
						if(Object[nIdx].m_nColorID != 1)
							continue;
					}
				}
				else
		...
		{
			ItemPos	Pos;
			int nWidth = Object[nIdx].m_nItemWidth;
			int nHeight = Object[nIdx].m_nItemHeight; 
		...
		int nX2 = 0;
		int nY2 = 0;
		// Xac dinh rang Object phai nam trong khu vuc
		Object[nIdx].GetMpsPos(&nX2, &nY2);
		int nDistanceObject = 0;
		nDistanceObject = GetDistanceMpx(nX1, nY1, nX2, nY2);
		if (nDistanceObject > nDistance)
			continue;

		int nX3 = 0;
		int nY3 = 0;
		Npc[nIndexPlayer].GetMpsPos(&nX3, &nY3);
		nDistancePlayer = GetDistanceMpx(nX2, nY2, nX3, nY3);
		if (nDistancePlayer <= nDistanceMin)
		{
			nDistanceMin = nDistancePlayer;
			nIndexReturn = nIdx;
		}
	}
