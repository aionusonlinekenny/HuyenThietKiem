...
void KProtocolProcess::s2cSyncAllSkill(BYTE * pMsg)
{
    ...
    // 1) Clear & n?p l?i skill list t? server (gi? nguy�n logic cu)
    Npc[nNpcIndex].m_SkillList.Clear();
    for (int i = 0; i < nSkillCount; i++)
    {
        if (pSync->m_sAllSkill[i].SkillId)
        {
            // N?u tru?c d�y b?n c� validate pOrdinSkill th� b?t l?i t?i d�y (tu?)
            Npc[nNpcIndex].m_SkillList.Add(
                pSync->m_sAllSkill[i].SkillId,
                pSync->m_sAllSkill[i].SkillLevel
            );
    ...
    // 2) RE-APPLY AURA RIGHT SKILL � g?i API public c?a KPlayer (kh�ng d?ng bi?n private)
#ifndef _SERVER
...
void KProtocolProcess::NpcWalkCommand(int nIndex, BYTE* pProtocol)
{
	...
	// FIX: Broadcast position to all clients in range to prevent desync
	// This fixes the "jumping" issue where other clients don't see smooth movement
