...
void KNpc::DoSkill(int nX, int nY)
{
	...
	// Broadcast to current region first with full quota
	int nMaxCount = MAX_BROADCAST_COUNT;
	CURREGION.BroadCast(&NetCommand, sizeof(NetCommand), nMaxCount, m_MapX, m_MapY);

	// Broadcast to adjacent regions, each with independent quota
	int i;
	for (i = 0; i < 8; i++)
	{
		if (CONREGIONIDX(i) == -1)
			continue;
		int nRegionMaxCount = MAX_BROADCAST_COUNT; // Reset quota for each adjacent region
		CONREGION(i).BroadCast(&NetCommand, sizeof(NetCommand), nRegionMaxCount, m_MapX - POff[i].x, m_MapY - POff[i].y);
	}
	m_HideState.nTime = 0;
#endif
...
void	KNpc::SendDataToNearRegion(void* pBuffer, DWORD dwSize)
{
	...
	for (int i= 0; i < 8; i++)
	{
		if (SubWorld[m_SubWorldIndex].m_Region[m_RegionIndex].m_nConnectRegion[i] < 0)
			continue;
		int nRegionMaxCount = MAX_BROADCAST_COUNT; // Reset quota for each adjacent region
		SubWorld[m_SubWorldIndex].m_Region[SubWorld[m_SubWorldIndex].m_Region[m_RegionIndex].m_nConnectRegion[i]].BroadCast(pBuffer, dwSize, nRegionMaxCount, m_MapX - POff[i].x, m_MapY - POff[i].y);
	}
}
#endif
