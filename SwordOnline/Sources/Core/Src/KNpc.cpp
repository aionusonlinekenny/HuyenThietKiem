...
#define	STUN_STATE_SPR					"\\spr\\skill\\����\\mag_spe_ѣ��.spr"
...
BOOL KNpc::ProcessState()
{
	...
	if(!(m_LoopFrames % 5))
	{
		// FIX: DISABLED periodic position broadcast for moving players
		// Position is already broadcasted in NpcWalkCommand/NpcRunCommand handlers
		// Broadcasting here causes POSITION CONFLICT - overwrites destination with current position
		// This causes "jumping back" effect where player is pulled back to old position every 5 frames
		// Only keep periodic broadcast for NPCs (monsters), not players
		/*
	...
	if (!(m_LoopFrames % GAME_UPDATE_TIME))
	{
		...
		if (m_Doing == do_sit)
		{
			int nLifeAdd = (m_CurrentLifeMax * 3 / 1000) * (1 + m_LifeReplenishPercent / 100);
			if (nLifeAdd <= 0)
				nLifeAdd = 1;
...
void KNpc::OnDeath()
{
	if (WaitForFrame())
	{
		...
		if (IsPlayer() && Npc[m_nLastDamageIdx].IsPlayer())
        {
                ...
                if (m_nPlayerIdx > 0)
                {
                        sprintf(szMessageDeath,"��i hi�p �� b� %s ��nh tr�ng th��ng",Npc[m_nLastDamageIdx].Name);
                ...
                {
                sprintf(szMessageDeath,"��i hi�p �� ��nh tr�ng th��ng %s",Name);
...
// FIX: REMOVED DUPLICATE BROADCAST - Already broadcasted in NpcRunCommand handler
// Broadcasting here causes duplicate messages to clients → movement stutter/reset
// Server broadcast is now only done in KProtocolProcess::NpcRunCommand()
/*
#ifdef _SERVER
...
// FIX: REMOVED DUPLICATE BROADCAST - Already broadcasted in NpcWalkCommand handler
// Broadcasting here causes duplicate messages to clients → movement stutter/reset
// Server broadcast is now only done in KProtocolProcess::NpcWalkCommand()
/*
#ifdef _SERVER
...
void KNpc::PaintNpcAuto(int nHeightOffset, bool bSelect, int nFontSize, DWORD dwBorderColor)
{
	...
	if (Player[CLIENT_PLAYER_INDEX].m_bActiveAuto && (Player[CLIENT_PLAYER_INDEX].m_nIndex == m_Index))
	{
		...
		{
			sprintf(szString, "[%s]", "T�m ��i t��ng...");
		}
		else
		{
			sprintf(szString, "[%s]", "�ang t� ��ng");
		}
		...
		{
			sprintf(szString, "[T�a ��:%d/%d]",nDesX, nDesY);
		}
...
int	KNpc::PaintWarning(int nHeightOffset, bool bSelect)
{
	...
	if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_bWarning) 
	{
		...
		)
		{
			...
			g_pRepresent->OutputText(12,"C� cao th� mai ph�c h�y th�n tr�ng!", KRF_ZERO_END, (nMpsXs - SHOW_LIFE_WIDTH * 2.5),nMpsYs, 0x810000, 0, nHeightOffset, 0);
		}
		return true;
	}
	return nRet;
...
void KNpc::SetStateSkillEffect(int nLauncher, int nSkillID, int nLevel, void *pData, int nDataNum, int nTime/* = -1*/, BOOL bRemove /*= FALSE*/)
{
    ...
    if (bRemove)
    {
        ...
        return; // kh�ng c� node d? remove
    }

    // ---------------- ADD / REFRESH / UPGRADE ----------------
    // T�m state c�ng SkillID d� t?n t?i
    pNode = (KStateNode*)m_StateSkillList.GetHead();
    while (pNode)
    {
        if (pNode->m_SkillID == nSkillID)
        {
            // REFRESH (c�ng level) -> gia h?n th?i gian
            if (pNode->m_Level == nLevel)
            {
                pNode->m_LeftTime = nTime;
#ifdef _SERVER
            ...
            else if (pNode->m_Level < nLevel)
            {
                ...
                for (int i = 0; i < nDataNum; i++)
                {
                    ...
                    // �p t�c d?ng m?i
                    ModifyAttrib(nLauncher, pTemp);
                    // luu gi� tr? d?o d?u d? ti?n g? sau n�y
                    pNode->m_State[i].nAttribType = pTemp->nAttribType;
                    pNode->m_State[i].nValue[0]   = -pTemp->nValue[0];
                    pNode->m_State[i].nValue[1]   = -pTemp->nValue[1];
                    pNode->m_State[i].nValue[2]   = -pTemp->nValue[2];
...
#ifdef _SERVER
    // N?u c� StateSpecialId -> th�m icon r?i broadcast
    if (pNode->m_StateGraphics)
    {
        AddStateInfo(pNode->m_StateGraphics);
        UpdateNpcStateInfo();
...
#ifdef _DEBUG
	//g_DebugLog("[��ֵ]AttackRating %d : Defense %d: RandomPercent (%d, %d)", nAR, nDf, nPercent, bRet);
#endif
	return bRet;
}
