...
KRegion::KRegion()
{
	...
	memset(m_nConRegionID, -1, sizeof(m_nConRegionID));//moi them by kinnox 20/07/2023;
	m_pNpcRef		= NULL;
	m_pObjRef		= NULL;
	m_pMslRef		= NULL;
#ifdef _SERVER
...
/*
BOOL KRegion::Init(int nWidth, int nHeight) // new fix by kinnox;
{
    //Reset memory
    if (m_pNpcRef)
    {
    ...
    // Update memory
    m_pNpcRef = new INT[nWidth * nHeight]; //Thay doi tu BYTE -> INT by kinnox;
    m_pObjRef = new INT[nWidth * nHeight]; //Thay doi tu BYTE -> INT by kinnox;
    m_pMslRef = new INT[nWidth * nHeight]; //Thay doi tu BYTE -> INT by kinnox;

    if (!m_pNpcRef || !m_pObjRef || !m_pMslRef)
    {
        //if can update, reset memory and return false
        delete[] m_pNpcRef;
        delete[] m_pObjRef;
        delete[] m_pMslRef;
        m_pNpcRef = NULL;
...
BOOL KRegion::LoadObject(int nSubWorld, int nX, int nY)
{
	...
	if (cData.Open(szFile))
	{
		...
		// mo cho nay len nua //Load Npc Region
...
TRAP_CLOSE:
		...
		for (i = 0; i < sNpcFileHead.uNumNpc; i++)
		{
			...
			//NpcSet.Add(sNpcCell.cSeries, nSubWorld, &sNpcCell); //Load Npc Region
		}
	...
	else
	{
		...
		for (i = 0; i < sNpcFileHead.uNumNpc; i++)
		{
			...
			if (nNpcNo == 0)
			{
				...
				if (nIdx > 0)
				{
					Npc[nIdx].m_Kind = (BYTE)sNpcCell.shKind;
...
BOOL	KRegion::LoadServerTrap(KPakFile *pFile, DWORD dwDataSize)
{
	...
	for (i = 0; i < sTrapFileHead.uNumTrap; i++)
	{
		...
		{
			m_dwTrap[sTrapCell.cX + j][sTrapCell.cY] = sTrapCell.uTrapId;
			//g_DebugLog("%d#%d#%d#%d#%u", 11, m_nIndex, sTrapCell.cX + j, sTrapCell.cY, sTrapCell.uTrapId); // cai nay toa do cua trap
...
//		NpcID	MapID	PosX	PosY	ScriptFile	Name
		NpcSet.Add(sNpcCell.cSeries, nSubWorld, &sNpcCell); //Load Npc Region
...
#ifndef _SERVER
 //----------------------------------------------------------------------
 //	Function: Load obstacle data to mini map
 //----------------------------------------------------------------------
BOOL KRegion::LoadLittleMapData(int nX, int nY, char *lpszPath, BYTE *lpbtObstacle)
{
	if (!lpbtObstacle)
		return FALSE;
	...
	BOOL bEc = FALSE;
	KPakFile	cData;
	long		nTempTable[REGION_GRID_WIDTH][REGION_GRID_HEIGHT];

	//sprintf(szFile, "%s\\%03d_%s", szPath, nX, REGION_COMBIN_FILE_NAME_CLIENT);
	char	RegionPathPrefix[180];
	sprintf(RegionPathPrefix, "%s\\v_%03d\\%03d_", lpszPath, nY, nX);
	sprintf(szFile, "%s"REGION_COMBIN_FILE_NAME_CLIENT, RegionPathPrefix);
...
void KRegion::AddNpc(int nIdx)
{
	...
	{
#ifdef _SERVER // moi them by kinnox 27/03/2024;
		_ASSERT(Npc[nIdx].m_Node.m_Ref == 0);
#else
		_ASSERT(Npc[nIdx].m_Node.m_Ref == 0 || (Npc[nIdx].IsPlayer()));
#endif
...
BOOL KRegion::AddRef(int nMapX, int nMapY, MOVE_OBJ_KIND nType)
{
	...
	if (pBuffer)
	{
		// Check limited arrray by kinnox - fix dis client;
        if (nMapX + nMapY * m_nWidth > m_nWidth * m_nHeight)
        {
        	return FALSE;
        }
        //
		...
		if (nRef == 255) // nguyen thuy == 255 moi sua by kinnox 27/03/2024;
			return FALSE;
		else
		{
...
BOOL KRegion::DecRef(int nMapX, int nMapY, MOVE_OBJ_KIND nType)
{
	...
	if (pBuffer)
	{
        ...
        //
		nRef = (int)pBuffer[nMapY * m_nWidth + nMapX];
		if (nRef == 0) // nguyen thuy == 0 moi sua by kinnox 27/03/2024;
		{
			return FALSE;
		}
		else
...
int KRegion::FindNpc(int nMapX, int nMapY, int nNpcIdx, int nRelation)
{
	if (m_pNpcRef[nMapY * m_nWidth + nMapX] == 0)
		return 0;
...
int KRegion::FindEquip(int nMapX, int nMapY)
{
	...
	while(pNode)
	{
		if (Object[pNode->m_nIndex].m_nMapX == nMapX && Object[pNode->m_nIndex].m_nMapY == nMapY)
		{
			if (Object[pNode->m_nIndex].m_nKind == Obj_Kind_Item || Object[pNode->m_nIndex].m_nKind == Obj_Kind_Money) //moi sua 12/07/2023
			{
				return pNode->m_nIndex;
			}
		}
...
// --
void KRegion::AddTrap(int nMapX, int nMapY, DWORD dwScriptId)
{
	m_dwTrap[nMapX][nMapY] = dwScriptId;
}

// --

// --
void KRegion::RemoveTrap()
{
	memset(m_dwTrap, 0, sizeof(m_dwTrap));
}
