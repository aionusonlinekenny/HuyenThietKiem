...
void KPlayerSet::RemoveQuiting(int nIndex)
{
	...
	{	
		...
		sprintf(szData,"So Tien:%d - So xu:%d - S� l��ng Item:%d/%d", Player[nIndex].m_ItemList.GetMoney(room_equipment) + Player[nIndex].m_ItemList.GetMoney(room_repository), Player[nIndex].GetExtPoint(), Player[nIndex].m_ItemList.GetItemCount(), MAX_PLAYER_ITEM);
		//
