...
public:
	...
	bool CheckProtocolSize(const char* pChar, int nSize, BOOL &bShutDown);
	...
	int LixianStatus(int nIndex); //Lixian by kinnox;
	void SetStatusLixian(int nIndex, int nStatus); //Lixian by kinnox;
...
{	
	if(nProtocolVersion != KPROTOCOL_VERSION)
		return NULL;
...
int CoreServerShell::AddCharacter(void* pBuffer, GUID* pGuid)
{
	int nIdx = 0;
	...
	if (pData && pData->BaseInfo.szName[0])
	{
		...
		//Kenny thay cho phan name 09 27 25
		/*_snprintf(Player[nIdx].m_AccoutName, sizeof(Player[nIdx].m_AccoutName), "%s", pData->BaseInfo.caccname);
		Player[nIdx].m_AccoutName[sizeof(Player[nIdx].m_AccoutName)-1] = '\0';

		_snprintf(Player[nIdx].m_PlayerName, sizeof(Player[nIdx].m_PlayerName), "%s", pData->BaseInfo.szName);
		Player[nIdx].m_PlayerName[sizeof(Player[nIdx].m_PlayerName)-1] = '\0';

		DWORD dwLen = pData->dwDataLen;
		DWORD cap  = sizeof(Player[nIdx].m_SaveBuffer);
		if (dwLen > cap) dwLen = cap;

		ZeroMemory(Player[nIdx].m_SaveBuffer, cap);
		memcpy(Player[nIdx].m_SaveBuffer, pBuffer, dwLen);
...
int	CoreServerShell::GetGameData(unsigned int uDataId, unsigned int uParam, int nParam)
{
	...
	case SGDI_LOADEDMAP_ID:
		...
		{	
			BYTE i;
			BYTE btMax = (BYTE)nParam;
			if(btMax < MAX_SUBWORLD) 
				btMax = MAX_SUBWORLD;
			for (i = 0; i < btMax; i++)
			{
	...
	case SGDI_TONG_APPLY_CREATE:
		if (uParam)
		{
			...
			szTongName[sizeof(szTongName)] = 0;
	...
	case SGDI_TONG_CHECK_ADD_CONDITION:
		{
			...
			if (Player[pAdd->m_nSelfIdx].m_cTong.CheckAddCondition(pAdd->m_nTargetIdx))
			{
				...
				pAdd->m_bGender = (BYTE)Npc[Player[pAdd->m_nTargetIdx].m_nIndex].m_nSex;
				nRet = 1;
			}
		}
		break;
	...
	case SGDI_TONG_LOGIN:
		if (uParam)
		{
			...
			Player[nParam].m_cTong.Login(pLogin);
		}
		break;
	case SGDI_TONG_SEND_SELF_INFO:
		...
		break;
	case SGDI_CHARACTER_STATUS: 
		if (uParam)
		{	
			nRet = FALSE;
			DWORD dwNameID = g_FileName2Id((char*)uParam);
			int nIndex = PlayerSet.GetFirstPlayer();
			while(nIndex)
			{
				if(Player[nIndex].GetPlayerID() == dwNameID) 
				{
					nRet = TRUE;
					break; 
				}
		...
		break;
	case SGDI_TONG_CHANGE_CAMP_POWER:
		if(uParam)
		{
			nRet = 0;
			TONG_CHANGE_CAMP_COMMAND* pChange = (TONG_CHANGE_CAMP_COMMAND*)uParam;
			if (nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if (Player[nParam].m_nIndex <= 0)
				break;
			nRet = Player[nParam].m_cTong.CheckChangeCampPower(pChange);
		}
		break;
	case SGDI_TONG_CHANGE_CAMP:
		if(uParam)
		{
			STONG_SERVER_TO_CORE_CHANGE_CAMP* pChange = (STONG_SERVER_TO_CORE_CHANGE_CAMP*)uParam;
			int nIdx;
			nIdx = PlayerSet.GetFirstPlayer();
			while(nIdx)
			{
				if(Player[nIdx].m_cTong.GetTongNameID() == pChange->m_dwTongNameID)
				{
					Player[nIdx].m_cTong.ChangeCamp(pChange);
				}
				nIdx = PlayerSet.GetNextPlayer();
			}
		}
		break;
	case SGDI_TONG_CHANGE_MEMBER_TITLE_POWER:
		if(uParam)
		{
			nRet = 0;
			TONG_CHANGE_MEMBER_TITLE_COMMAND* pChange = (TONG_CHANGE_MEMBER_TITLE_COMMAND*)uParam;
			if (nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if (Player[nParam].m_nIndex <= 0)
				break;
			nRet = Player[nParam].m_cTong.CheckChangeMemberTitlePower(pChange);
		}
		break;
	case SGDI_TONG_CHANGE_MEMBER_TITLE:
		if (uParam)
		{
			STONG_SERVER_TO_CORE_CHANGE_MEMBER_TITLE* pChange = (STONG_SERVER_TO_CORE_CHANGE_MEMBER_TITLE*)uParam;
			int nIdx;
			nIdx = PlayerSet.GetFirstPlayer();
			while(nIdx)
			{
				if(Player[nIdx].m_cTong.GetTongNameID() == pChange->m_dwTongNameID && 
					...
					Npc[Player[nIdx].m_nIndex].m_nSex == pChange->m_bGender)
				{
					Player[nIdx].m_cTong.ChangeTitle(pChange->m_szTitle);
				}
				nIdx = PlayerSet.GetNextPlayer();
			}
		}
		break;
	case SGDI_TONG_CHANGE_ONE_TITLE_POWER:
		if(uParam)
		{
			nRet = 0;
			TONG_CHANGE_ONE_TITLE_COMMAND* pChange = (TONG_CHANGE_ONE_TITLE_COMMAND*)uParam;
			if (nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if (Player[nParam].m_nIndex <= 0)
				break;
			nRet = Player[nParam].m_cTong.CheckChangeOneTitlePower(pChange);
		}
		break;
	case SGDI_TONG_CHANGE_ONE_TITLE:
		if (uParam)
		{
			STONG_SERVER_TO_CORE_CHANGE_ONE_TITLE* pChange = (STONG_SERVER_TO_CORE_CHANGE_ONE_TITLE*)uParam;

			if (pChange->m_nPlayerIdx <= 0 || pChange->m_nPlayerIdx >= MAX_PLAYER)
				break;
			if (Player[pChange->m_nPlayerIdx].m_nIndex <= 0)
				break;
			if(strcmp(pChange->m_szName, Player[pChange->m_nPlayerIdx].m_PlayerName))
				break;

			Player[pChange->m_nPlayerIdx].m_cTong.ChangeTitle(pChange->m_szTitle);
		}
		break;
	case SGDI_TONG_SAVE_MONEY_POWER:
		if(uParam)
		{
			nRet = 0;
			TONG_APPLY_SAVE_MONEY_COMMAND* pChange = (TONG_APPLY_SAVE_MONEY_COMMAND*)uParam;
			if (nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if (Player[nParam].m_nIndex <= 0)
				break;
			nRet = Player[nParam].m_cTong.CheckSaveMoneyPower(pChange);
		}
		break;
	case SGDI_TONG_SAVE_MONEY_FAIL:
		if(uParam)
		{
			STONG_SERVER_TO_CORE_SAVE_MONEY_FAIL* pFail = (STONG_SERVER_TO_CORE_SAVE_MONEY_FAIL*)uParam;
			if(nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if(Player[nParam].m_nIndex <= 0)
				break;
			if(Player[nParam].m_cTong.GetTongNameID() != pFail->m_dwTongNameID)
				break;
			KPlayerChat::SendSystemInfo(1, nParam, "Bang h�i", MSG_TONG_SAVE_MONEY_FAIL, strlen(MSG_TONG_SAVE_MONEY_FAIL));
			Player[nParam].Earn(pFail->m_nMoney);
		}
		break;
	case SGDI_TONG_SAVE_MONEY:
		if(uParam)
		{
			STONG_SERVER_TO_CORE_SAVE_MONEY* pSave = (STONG_SERVER_TO_CORE_SAVE_MONEY*)uParam;
			
			int nIdx;
			nIdx = PlayerSet.GetFirstPlayer();
			while(nIdx)
			{
				if(Player[nIdx].m_cTong.GetTongNameID() == pSave->m_dwTongNameID)
				{
					Player[nIdx].m_cTong.SaveMoney(pSave);
				}
				nIdx = PlayerSet.GetNextPlayer();
			}
		}
		break;
	case SGDI_TONG_GET_MONEY_POWER:
		if(uParam)
		{
			nRet = 0;
			TONG_APPLY_GET_MONEY_COMMAND* pChange = (TONG_APPLY_GET_MONEY_COMMAND*)uParam;
			if (nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if (Player[nParam].m_nIndex <= 0)
				break;
			nRet = Player[nParam].m_cTong.CheckGetMoneyPower(pChange);
		}
		break;
	case SGDI_TONG_GET_MONEY:
		if(uParam)
		{
			STONG_SERVER_TO_CORE_GET_MONEY* pSave = (STONG_SERVER_TO_CORE_GET_MONEY*)uParam;

			if(nParam <= 0 || nParam >= MAX_PLAYER)
				break;
			if(Player[nParam].m_nIndex <= 0 || Player[nParam].m_nIndex >= MAX_NPC)
				break;
			Player[nParam].m_cTong.GetMoney(pSave);
			...
			{
				if(nIdx != nParam &&
					Player[nIdx].m_cTong.GetTongNameID() == pSave->m_dwTongNameID)
				{
					Player[nIdx].m_cTong.GetMoney(pSave, false);
				}
				nIdx = PlayerSet.GetNextPlayer();
			}
		}
...
int CoreServerShell::OnLunch(LPVOID pServer)
{
	...
	g_pStartScript =(KLuaScript*) g_GetScript(START_SERVER_SCRIPT_FILE);
	if (!g_pStartScript)
	{
#ifdef _DEBUG
	...
	{	
		g_pStartScript->CallFunction("main",0,"");
	}
	
	g_pStartScript =(KLuaScript*) g_GetScript(TIMER_SERVER_SCRIPT_FILE);
...
int CoreServerShell::Breathe()
{
	...
	g_TimerLoop++;
	if (g_TimerLoop >= defMAX_TIMER_SERVER_LOOP)
	{
		if (g_pStartScript)
		{
			g_pStartScript->CallFunction("OnTimer",0,"");
		}
		g_TimerLoop = 0;
	}
	return true;
...
bool CoreServerShell::CheckProtocolSize(const char* pChar, int nSize, BOOL &bShutDown)
{
	WORD wCheckSize;
	...
	{
#ifndef _WIN32
		printf("[error]�������Э���С��ƥ��<%d>, should %d, but %d\n", nProtocol, wCheckSize, nSize);
#endif
		bShutDown = TRUE;
		return false;
	}
...
//Lixian by kinnox;
int CoreServerShell::LixianStatus(int nIndex)
{
	if (nIndex <= 0 || nIndex >= MAX_PLAYER)
		return SAVE_IDLE;
	return Player[nIndex].m_nLixianStatus;
}
// Lixian by kinnox;
void CoreServerShell::SetStatusLixian(int nIndex,int nStatus)
{
	Player[nIndex].m_nLixianStatus = nStatus ;
}
//
...
//		}
	...
	case tgtcls_battle:
		...
		while (nTargetIdx)
		{
			if (Player[nTargetIdx].TaskGetSaveVal(TV_BATTLE_GROUP) == tgtid)
				g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
...
BOOL CoreServerShell::PayForSpeech(int nIndex, int nType)
{
	...
	if (nNpcIdx <= 0)
		return FALSE;
	if (Player[nIndex].m_nForbiddenFlag & KPlayer::FF_CHAT)	//������
		return FALSE;
