...
public:
	...
	static int FindServerByAccount(const char* szAcc);

	virtual bool Attach( const char *pAccountName, bool bCheck = false );		//
	...
	static void BeginTransfer(const char* acc);
    static void EndTransfer(const char* acc);
    static bool IsInTransfer(const char* acc);
	static void CleanupStaleTransfers();
protected:
	...
	bool _SyncRoleInfo( const void *pData, size_t datalength );
	...
	bool FreezeMoney( const char *pAccountName );
private:
	...
	static OnlineGameLib::Win32::CCriticalSection m_csTransfer;
    typedef std::set<std::string> TransferSet;
    static TransferSet m_transferAccs;
private:
...
//	IClient			*m_pAccountClient;
	...
	bool AttatchAccountToGameServer( const char *pAccountName, bool bCheck = false );		//
