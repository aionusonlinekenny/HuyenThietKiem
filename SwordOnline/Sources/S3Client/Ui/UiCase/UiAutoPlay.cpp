...
#define		MSG_NON_SETTINGS		"Kh�ng thi�t l�p"
...
void KUiRecovery::LoadScheme(KIniFile* pIni)
{
	...
	m_cMessenger = " (xh)<color=orange>V� L�m Truy�n K� <color> (dd)<color=red>AutoPlay<color> ";
...
//	"B� qua n� tr�nh(%)", "ignoredefense_p",
	...
	"TGian ch�m(%)", 	"freezetimereduce_p",
	"TGian tr�ng ��c(%)", "poisontimereduce_p",
	"TGian cho�ng(%)", 	"stuntimereduce_p",
	"TGian ph�c h�i(%)", 	"fasthitrecover_v",
...
char Data_Magic[defMAX_AUTO_FILTERL][2][64] = 
{
	...
	"H�t Sl %", 	"steallifeenhance_p",
	"H�t Nl %", 	"stealmanaenhance_p",
	"Uy danh h", 		"addtitle_v",
	"S�c m�nh �", 		"strength_v",
	"Th�n ph�p �", 	"dexterity_v",
	"Sinh kh� �", 		"vitality_v",
...
//	"B� qua n� tr�nh-%", "ignoredefense_p",
	"Ch�nh x�c �", 	"attackrating_v",
	"STVL n�i �", 	"addphysicsmagic_v",
	"BS n�i �", 	"addcoldmagic_v",
	"HS n�i �", 	"addfiremagic_v",
	"LS n�i �", 	"addlightingmagic_v",
	"�S n�i �", 	"addpoisonmagic_v",
	"TGian ch�m %", 	"freezetimereduce_p",
...
{
	// Send auto-sort toggle to server (protocol + mode)
	// mode: 0 = disable auto-sort on pickup, 1 = enable auto-sort on pickup
	if (g_pClient)
	{
		BYTE btFlag = 0;
		...
		g_DebugLog("[CLIENT] OnAutoSortClick: Sent auto-sort %s (protocol=%d, mode=%d)",
			...
			else if (HIWORD(nParam) == MENU_SELECT_JPT && (short)(LOWORD(nParam)) >= 0)
			{
				...
				{	
					if (m_ListName[0][0] == NULL)
					{
						...
						strcpy(Msg.szMessage, "Danh s�ch tr�ng th�a ��i hi�p!");
...
void KUiOther::MenuListParty()
{
	...
	else 
	{
		...
		Msg.eType = SMT_NORMAL;
		Msg.uReservedForUi = 0;
		strcpy(Msg.szMessage, "N�i n�y ��ng kh�ng m�ng qu�nh, kh�ng m�t b�ng ng��i!");
		KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
	}
}
		...
		else if (uParam == (unsigned int)(KWndWindow*)&c_MoveAround)
		{
			...
			{	
				...
				else
				{
					...
					Msg.eType = SMT_NORMAL;
					Msg.uReservedForUi = 0;
					strcpy(Msg.szMessage, "T�a �� ch� ��nh ch�a thi�t l�p!");
					KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
				}
			}
			else
