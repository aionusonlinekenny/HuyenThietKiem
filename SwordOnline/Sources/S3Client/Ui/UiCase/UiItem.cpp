...
#define	SELL_MESSAGES	"�n gi� <color=green>Shift + chu�t ph�i<color> v�o v�t ph�m, l�p t�c b�n ���c v�t ph�m."
...
void KUiItem::CloseWindow(bool bDestroy)
{
	if (m_pSelf)
	{
		...
		// This makes ESC key work indirectly (ESC closes inventory -> crafting UI also closes)
...
void KUiItem::MoveItemToRepository(KUiDraggedObject* pItem)
{
    ...
    if (pStoreBox)
    {
        ...
        if (curPage >= numOpened)
        {
			...
			Msg.eType = SMT_NORMAL;
			Msg.uReservedForUi = 0;
			strcpy(Msg.szMessage, "B�n ch�a m� r��ng ch�a n�y !");
			KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
			return;
        }
        ...
        if (containerID < UOC_STORE_BOX || containerID > UOC_STORE_BOX5)
        {
            ...
            Msg.eType = SMT_NORMAL;
            Msg.uReservedForUi = 0;
            strcpy(Msg.szMessage, "R��ng �ang m� kh�ng c�n ch� tr�ng!");
            KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
            return;
        }
...
void KUiItem::MoveItemToInventory(KUiDraggedObject* pItem)
{
    ...
    // container hi?n t?i x�c ??nh theo t?ng page m?
    KUiStoreBox* pStoreBox = KUiStoreBox::GetIfVisible();
    if (!pStoreBox)
    {
        KSystemMessage Msg;
    ...
    if (containerID < UOC_STORE_BOX || containerID > UOC_STORE_BOX5)
    {
        ...
        Msg.eType = SMT_NORMAL;
        Msg.uReservedForUi = 0;
        strcpy(Msg.szMessage, "R��ng �ang m� kh�ng h�p l� !");
        KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
        return;
    }
