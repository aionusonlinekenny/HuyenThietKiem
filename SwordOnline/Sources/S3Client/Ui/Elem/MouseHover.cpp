			...
			unsigned int uId, int nContainer, int x, int y)
{
	...
	if (g_pCoreShell)
	{
		...
		if(uGenre == CGOG_ITEM || uGenre == CGOG_NPCSELLITEM || uGenre == CGOG_PLAYERSELLITEM)
		{	
			char szIconName[80];
			if(g_pCoreShell->GetGameData(GDI_ITEM_GENRE, (unsigned int)&Obj, 0) == 0)
			{
				if(g_pCoreShell->GetGameData(GDI_ITEM_DETAIL, 0, (int)&Obj) == 11)
				{
