...
static void ReleaseLoginLockSafe(const std::string &acc)
{
    ...
    printf("[BISHOP] Safe unlock for %s\n", acc.c_str());
}
/*
...
bool CGamePlayer::Inactive()
{
    ...
    // Safe-lock v?i AccServer: luôn release khi k?t thúc phiên
...
UINT CGamePlayer::WaitForAccPwd()
{
    ...
    if ( pRetBuffer )
    {
        ...
        if (!pLAI)
        {
            LoginLog("[WaitForAccPwd][ID=%ld] ERROR: pLAI null", m_lnIdentityID);
            _VerifyAccount_ToPlayer(LOGIN_A_LOGIN | LOGIN_R_TIMEOUT, 0);
            SAFE_RELEASE(pRetBuffer);
            m_theDataQueue[enumOwnerPlayer].Detach(c2s_login);
            return enumError;
        }

        // Field dúng theo struct th?c t?
        const char *pAccName = pLAI->Account;
        const char *pAccPwd  = pLAI->Password.szPassword;
        const char *pLicense = pLAI->License;

        // Trim th? công (không c?n <ctype.h>)
        char accBuf[64], pwdBuf[64], licBuf[64];
        accBuf[0] = pwdBuf[0] = licBuf[0] = '\0';

        // account
        {
            const char* src = pAccName ? pAccName : "";
            const char* a = src;
            while (*a==' '||*a=='\t'||*a=='\r'||*a=='\n') ++a;
            const char* b=a; while(*b) ++b;
            while(b>a && (b[-1]==' '||b[-1]=='\t'||b[-1]=='\r'||b[-1]=='\n')) --b;
            int n=(int)(b-a); if(n>(int)sizeof(accBuf)-1) n=(int)sizeof(accBuf)-1;
            if(n>0) memcpy(accBuf,a,n); accBuf[n]='\0';
        }
        // password
        {
            const char* src = pAccPwd ? pAccPwd : "";
            const char* a = src;
            while (*a==' '||*a=='\t'||*a=='\r'||*a=='\n') ++a;
            const char* b=a; while(*b) ++b;
            while(b>a && (b[-1]==' '||b[-1]=='\t'||b[-1]=='\r'||b[-1]=='\n')) --b;
            int n=(int)(b-a); if(n>(int)sizeof(pwdBuf)-1) n=(int)sizeof(pwdBuf)-1;
            if(n>0) memcpy(pwdBuf,a,n); pwdBuf[n]='\0';
        }
        // license
        {
            const char* src = pLicense ? pLicense : "";
            const char* a = src;
            while (*a==' '||*a=='\t'||*a=='\r'||*a=='\n') ++a;
            const char* b=a; while(*b) ++b;
            while(b>a && (b[-1]==' '||b[-1]=='\t'||b[-1]=='\r'||b[-1]=='\n')) --b;
            int n=(int)(b-a); if(n>(int)sizeof(licBuf)-1) n=(int)sizeof(licBuf)-1;
            if(n>0) memcpy(licBuf,a,n); licBuf[n]='\0';
        }
        ...
        if (nCheckProtocolVersion)
        {
            if (accBuf[0] != '\0')
            {
                // Acquire TEMP-LOCK ngay t?i dây
                std::string _acc(accBuf);
                if (!AcquireTempLock(_acc))
                {
                    LoginLog("[WaitForAccPwd][ID=%ld] TEMP-LOCK FAILED for \"%s\" -> duplicate/pending", m_lnIdentityID, accBuf);
                ...
                // chuy?n sang task g?i AccServer
                nNextTask = enumToNextTask;

                LoginLog("[WaitForAccPwd][ID=%ld] Ready to send AccountLoginInfo for \"%s\" (next=QueryAccPwd)",
        ...
        else
        {
            ...
            LoginLog("[WaitForAccPwd][ID=%ld] Invalid protocol for \"%s\"", m_lnIdentityID, m_sAccountName.c_str());
        }
        ...
        case ACTION_SUCCESS:
            {
                ...
                bool alreadyOnline = (g_ActiveAccounts.find(m_sAccountName) != g_ActiveAccounts.end());

                LoginLog("[VerifyAccount][ID=%ld] alreadyOnline=%d inTransfer=%d",
                         m_lnIdentityID, alreadyOnline ? 1 : 0, inTransfer ? 1 : 0);

                if (alreadyOnline && !inTransfer)
                {
                    // Duplicate - KHÔNG unlock ACTIVE
                    LoginLog("[VerifyAccount][ID=%ld] DUPLICATE rejected for \"%s\" (owner unchanged)",
                             m_lnIdentityID, m_sAccountName.c_str());
