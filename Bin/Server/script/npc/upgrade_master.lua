...
function ExeUpgradeAttrib()
    ...
    -- Get generator levels (tiers) from the old item - THIS is what client uses!
    local l0, l1, l2, l3, l4, l5 = GetItemGeneratorLevels(nEquipIdx)
    local nGenLevels = {l0, l1, l2, l3, l4, l5}

    Msg2Player("Old generator levels (tiers): [" .. l0 .. "," .. l1 .. "," .. l2 .. "," .. l3 .. "," .. l4 .. "," .. l5 .. "]")

    -- CRITICAL: We must upgrade the GENERATOR LEVEL (tier), not the attribute value!
    -- Client regenerates attributes from generator levels, so setting values manually won't work
    local nOldTier = nGenLevels[nAttribSlot+1]
    local nNewTier = nOldTier + 1  -- Upgrade to next tier

    -- Cap at tier 10 (max)
    if nNewTier > 10 then
        nNewTier = 10
        Msg2Player("WARNING: Already at max tier (10)")
    end

    nGenLevels[nAttribSlot+1] = nNewTier
    Msg2Player("Upgrading slot " .. nAttribSlot .. " tier: " .. nOldTier .. " -> " .. nNewTier ..
               " (value will be generated by MagicAttrib.txt)")
    ...
    -- Create new item with upgraded generator levels (client will regenerate attributes from these)
    Msg2Player("Creating new item with upgraded generator levels...")
    Msg2Player("Genre=" .. nGenre .. " Detail=" .. nDetail .. " Parti=" .. nParti .. " Level=" .. nLevel .. " Series=" .. nSeries .. " Luck=" .. nLuck)
    Msg2Player("Generator Levels: [" .. nGenLevels[1] .. "," .. nGenLevels[2] .. "," .. nGenLevels[3] .. "," ..
                                       nGenLevels[4] .. "," .. nGenLevels[5] .. "," .. nGenLevels[6] .. "]")

    -- Use AddItemEx to create item with specific generator levels
    AddItemEx(
        nGenre, nDetail, nParti, nLevel, nSeries, nLuck,
        nGenLevels[1], nGenLevels[2], nGenLevels[3],
        nGenLevels[4], nGenLevels[5], nGenLevels[6],
        1, 0,  -- version=1, randseed=0
        nPos   -- pos_builditem
    )

    Msg2Player("New item created successfully! Client will regenerate attributes from generator levels.")
