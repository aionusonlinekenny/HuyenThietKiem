...
function main()
	print("TRAP 4 - Qua Thanh Thành Sơn (Map 21)");
	...
	if nTask > 0 and nTask < 4 then
		...
		if dwCartID > 0 then
			...
			if nNpcIdx > 0 then
				...
				-- Delete old cart
				DelNpc(nNpcIdx)
				print("Vận Tiêu: Deleted old cart at map 11")
			end
		end
	end

	-- Teleport player to Thanh Thành Sơn (map 21)
	if (NewWorld(21, 2625, 4507) == 1) then
		...
		if nCartType > 0 and nCartTemplateID > 0 then
			...
			if nSubWorldIdx >= 0 then
				...
				local nCartX = 2625 * 32 + 64  -- +2 tiles offset
				local nCartY = 4507 * 32
				...
				if nId > 0 then
					...
					Msg2Player("Tiêu xa đã theo bạn qua Thanh Thành Sơn!")
					print("Vận Tiêu: Spawned new cart at map 21, NPC ID: " .. dwNewCartID)
				else
					Msg2Player("Lỗi: Không thể tạo tiêu xa ở Thanh Thành Sơn!")
