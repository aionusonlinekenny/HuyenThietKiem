----------------------------------------
--Script Trap qua maps - Modified for V?n Ti�u support
--Code By Kinnox
----------------------------------------
...
function main()
	...
	print("TRAP 1 - Quay v? D??ng Ch�u (Map 11)");
	...
	-- Teleport player back to D??ng Ch�u (map 11)
	if (NewWorld(11, 2845,4811) == 1) then
		...
		if nNpcIdx > 0 then
			DelNpc(nNpcIdx)
			print("V?n Ti�u: Deleted old cart at map 21")
		end
		...
		if nCartType > 0 and nCartTemplateID > 0 then
			...
			if nSubWorldIdx >= 0 then
				...
				if nId > 0 then
					...
					Msg2Player("Ti�u xa d� theo b?n v? D??ng Ch�u!")
				else
					Msg2Player("L?i: Kh�ng th? t?o ti�u xa ? D??ng Ch�u!")
				end
			end
		end
	end
